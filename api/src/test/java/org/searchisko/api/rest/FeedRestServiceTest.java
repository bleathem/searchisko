...
public class FeedRestServiceTest {
	...
	public void patchQuerySettings() {
		...
		// case - set and reset some values on nonempty QuerySettings
		{
			...
			Assert.assertEquals(0, qs.getFacets().size());
			Assert.assertEquals("ct", qs.getFilters().getFilterCandidateValues(CONTENT_TYPE_KEY));
			Assert.assertEquals("cp", qs.getFilters().getFilterCandidateValues(SYS_CONTENT_PROVIDER));
	...
	public void constructFeedTitle() throws URISyntaxException {
		...
		{
			QuerySettings querySettings = new QuerySettings();
			querySettings.getFiltersInit().acknowledgeUrlFilterCandidate(CONTENT_TYPE_KEY,"conetnt_type");
		...
		// case - list param
		{
			...
			Assert.assertEquals("Feed content for criteria project=[as7]", tested.constructFeedTitle(querySettings)
					.toString());
			querySettings.getFiltersInit().acknowledgeUrlFilterCandidate(PROJECTS_KEY, "aerogear");
		...
		// case - multiple params
		{
			...
			querySettings.getFiltersInit().acknowledgeUrlFilterCandidate(PROJECTS_KEY, "as7");
			querySettings.getFiltersInit().acknowledgeUrlFilterCandidate(PROJECTS_KEY, "aerogear");
			querySettings.getFiltersInit().acknowledgeUrlFilterCandidate(TAGS_KEY, "tag1");
			querySettings.getFiltersInit().acknowledgeUrlFilterCandidate(TAGS_KEY, "tag2");
			...
			querySettings.getFiltersInit().acknowledgeUrlFilterCandidate(SYS_TYPES_KEY, "issue");
			querySettings.getFiltersInit().acknowledgeUrlFilterCandidate(SYS_TYPES_KEY, "blogpost");
