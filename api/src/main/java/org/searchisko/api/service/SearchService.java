...
public class SearchService {
	...
	public SearchResponse performSearch(QuerySettings querySettings, String responseUuid, StatsRecordType statsRecordType) {
		try {
			...
			Map<String, FilterBuilder> searchFilters = prepareFilters(querySettings);
	...
	protected void setSearchRequestIndicesAndTypes(QuerySettings querySettings, SearchRequestBuilder srb) {
		...
		if (contentTypes != null && contentTypes.size() > 0) {
			...
			for (String type : contentTypes) {
				...
				if (typeDef == null) {
					throw new IllegalArgumentException("type"); // TODO:
				}
	 ...
	 */
	protected Map<String, FilterBuilder> prepareFilters(QuerySettings querySettings) {
		QuerySettings.Filters filters = querySettings.getFilters();
		Map<String, FilterBuilder> searchFilters = new LinkedHashMap<>();

		if (filters != null) {

			Map<String, SemiParsedFilterConfig> semiParsedFilters = parsedFilterConfigService.getFilterConfigsForRequest();
			...
			for (SemiParsedFilterConfig filterConfig : semiParsedFilters.values()) {
				...
				if (filterConfig instanceof SemiParsedTermsFilterConfig) {
					...
					Set<String> fn = parsedFilterConfigService.getFilterNamesForDocumentField(conf.getFieldName());
					...
					// TODO: handle terms filter <optional_settings>
				...
				// range filter
					...
					if (searchFilters.containsKey(conf.getFieldName()) &&
							searchFilters.get(conf.getFilterName()) instanceof RangeFilterBuilder) {
						// in this case we will be adding (or overriding) settings of existing filter
						rfb = (RangeFilterBuilder) searchFilters.get(conf.getFilterName());
					} else {
						rfb = new RangeFilterBuilder(conf.getFieldName());
					}

					Set<String> fn = parsedFilterConfigService.getFilterNamesForDocumentField(conf.getFieldName());
					final Set<String> filterValues = filters.getFilterCandidateValues(fn);
					if (conf.definesGte()) {
						if (filterValues.size() > 0) {
							long time = DATE_TIME_FORMATTER_UTC.parseDateTime(filterValues.iterator().next()).toDate().getTime();
							rfb.gte(time);
						}
					...
					// handle <_enum> if specified
					if (conf.getEnum() != null) {

					}

					// TODO: handle range filter <optional_settings>
					searchFilters.put(conf.getFieldName(), rfb);

				} else {
					// unknown parsed type, ignore for now
				}
			}
...
//			}
	...
	*/
	...
	protected static String selectActivityDatesHistogramInterval(QuerySettings querySettings) {
		...
		// TODO: no hardcodes
		String ACTIVITY_DATE_INTERVAL = "activity_date_interval";
		String ACTIVITY_DATE_FROM = "activity_date_from";
		String ACTIVITY_DATE_TO = "activity_date_to";
		if (filters != null) {
			if (filters.getFilterCandidatesKeys().contains(ACTIVITY_DATE_INTERVAL)) {
				String value = filters.getFirstValueForFilterCandidate(ACTIVITY_DATE_INTERVAL);
				PastIntervalValue intervalValue = value != null ? PastIntervalValue.parseRequestParameterValue(value, ACTIVITY_DATE_INTERVAL) : null;
			...
			} else if (filters.getFilterCandidatesKeys().contains(ACTIVITY_DATE_FROM) ||
					filters.getFilterCandidatesKeys().contains(ACTIVITY_DATE_TO)) {
				long from = 0;
				long to = 0;
				if (filters.getFilterCandidatesKeys().contains(ACTIVITY_DATE_TO)) {
					to = DATE_TIME_FORMATTER_UTC.parseDateTime(filters.getFirstValueForFilterCandidate(ACTIVITY_DATE_TO)).toDate().getTime();
				} else {
					to = System.currentTimeMillis();
				}
				if (filters.getFilterCandidatesKeys().contains(ACTIVITY_DATE_FROM)) {
					from = DATE_TIME_FORMATTER_UTC.parseDateTime(filters.getFirstValueForFilterCandidate(ACTIVITY_DATE_FROM)).toDate().getTime();
				}
